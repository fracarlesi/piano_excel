/**
 * French With Grace Volumes Calculator
 * 
 * Calcola i nuovi volumi erogati per prestiti francesi con periodo di grazia
...
/**
 * Calculate new volumes for french loans with grace period
 ...
 */
export const calculateFrenchWithGraceVolumes = (product, assumptions, quarters = 40) => {
  const results = {
    ...
    metrics: {
      ...
      gracePeriodQuarters: product.gracePeriod || 0
    }
  };
    ...
    if (yearVolume > 0) {
      ...
      quarterlyAllocation.forEach((percentage, quarter) => {
        ...
        if (quarterIndex < quarters) {
          // Volume is already in millions of euros, just apply percentage
          ...
          if (quarterlyVolume > 0) {
            ...
            // Create vintage detail with grace period info
            results.vintages.push({
              ...
              amortizationType: 'french-with-grace',
              firstAmortizationQuarter: quarterIndex + (product.gracePeriod || 0)
            });
          }
        }
      });
