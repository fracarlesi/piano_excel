...
export const calculateStockNBVPerforming = (
  ...
  if (totalNBVResults && totalNBVResults.byProduct && gbvDefaultedResults && gbvDefaultedResults.byProduct) {
    Object.entries(totalNBVResults.byProduct).forEach(([productKey, productNBV]) => {
      ...
      console.log(`  - GBV Defaulted Q0: €${gbvDefaulted[0]?.toFixed(1)}M`);
  ...
  console.log('\n📊 Stock NBV Performing Calculator - Summary');
  console.log(`  - Total products processed: ${Object.keys(results.byProduct).length}`);
  console.log(`  - Average Stock NBV Performing: €${results.consolidatedMetrics.averageStockNBVPerforming.toFixed(1)}M`);
  console.log(`  - Growth rate (CAGR): ${results.consolidatedMetrics.growthRate.toFixed(2)}%`);
  console.log(`  - Stock NBV Performing Q0: €${results.balanceSheetLine.quarterly[0].toFixed(1)}M`);
  console.log(`  - Stock NBV Performing Q${quarters-1}: €${results.balanceSheetLine.quarterly[quarters-1].toFixed(1)}M`);
