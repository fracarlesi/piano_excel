/**
 * French With Grace NBV Calculator
 * 
 * Calcola il Net Book Value totale dei prestiti French con periodo di grazia
 * Include tutta la logica di ammortamento alla francese con preammortamento
 */
...
/**
 * Calculate total NBV for French Loans with grace period
 ...
 */
export const calculateFrenchWithGraceNBV = (product, assumptions, quarters = 40) => {
  const results = {
...
/**
 * Create vintages for french loans with grace period
 * @private
 */
const createFrenchWithGraceVintages = (product, yearlyVolumes, assumptions) => {
  ...
  const gracePeriod = product.gracePeriod || 4; // Default 1 year grace
    ...
    if (volume > 0) {
      ...
      quarterlyAllocation.forEach((percentage, quarter) => {
        ...
        if (quarterlyVolume.gt(0)) {
          const vintage = {
            id: `french_wg_${product.id || product.name}_y${year}_q${quarter}`,
            type: 'french',
            subType: 'with-grace',
          ...
          // Calculate quarterly payment after grace period
          // Post-grace periods = total duration - grace period
          const postGracePeriods = vintage.durationQuarters - vintage.gracePeriod;
          vintage.quarterlyPaymentPostGrace = calculateQuarterlyPayment(
