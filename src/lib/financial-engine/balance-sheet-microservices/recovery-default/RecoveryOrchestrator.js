...
export const calculateRecoveryDefault = (divisions, assumptions, quarters = 40, totalAssetsResults = null) => {
  ...
  // Debug final results
  console.log('🔍 Recovery Calculator Debug - Final Results');
  console.log('  - Total expected recovery:', results.consolidatedMetrics.totalExpectedRecovery);
  console.log('  - Recovery in first 10 quarters:', results.balanceSheetLine.quarterly.slice(0, 10));
  console.log('  - Products with recovery:', Object.keys(results.byProduct));
...
const calculateProductRecovery = (productConfig, defaultFlows, assumptions, quarters) => {
  ...
  defaultFlows.forEach((defaultAmount, quarter) => {
    if (defaultAmount > 0) {
      console.log(`📊 Processing default: Q${quarter + 1} = €${defaultAmount.toFixed(1)}M`);
      ...
      if (stateGuaranteeFirst && collateralFirst) {
        if (stateGuaranteeFirst.quarter <= collateralFirst.quarter) {
          ...
          if (cappedStateGuarantee < stateGuaranteeRecovery.recoveryAmount || cappedCollateral < collateralRecovery.netRecovery) {
            wasCapped = true;
            console.log(`⚠️ Dynamic capping applied:`);
            console.log(`   → GBV Defaulted: €${defaultAmount.toFixed(1)}M`);
            console.log(`   → State Guarantee (Q${stateGuaranteeFirst.quarter + 1}): €${stateGuaranteeRecovery.recoveryAmount.toFixed(1)}M → €${cappedStateGuarantee.toFixed(1)}M`);
            console.log(`   → Remaining after SG: €${remainingGBVDefaulted.toFixed(1)}M`);
            console.log(`   → Collateral (Q${collateralFirst.quarter + 1}): €${collateralRecovery.netRecovery.toFixed(1)}M → €${cappedCollateral.toFixed(1)}M`);
          }
        } else {
          ...
          if (cappedCollateral < collateralRecovery.netRecovery || cappedStateGuarantee < stateGuaranteeRecovery.recoveryAmount) {
            ...
            console.log(`   → GBV Defaulted: €${defaultAmount.toFixed(1)}M`);
            console.log(`   → Collateral (Q${collateralFirst.quarter + 1}): €${collateralRecovery.netRecovery.toFixed(1)}M → €${cappedCollateral.toFixed(1)}M`);
            console.log(`   → Remaining after COL: €${remainingGBVDefaulted.toFixed(1)}M`);
            console.log(`   → State Guarantee (Q${stateGuaranteeFirst.quarter + 1}): €${stateGuaranteeRecovery.recoveryAmount.toFixed(1)}M → €${cappedStateGuarantee.toFixed(1)}M`);
          }
        }
      ...
      console.log(`💰 Recovery for Q${quarter + 1} default: ${recoveryTiming.schedule.length} cashflows generated`);
      recoveryTiming.schedule.forEach((cf, i) => {
        if (cf.totalRecovery > 0) {
          console.log(`  - Q${cf.quarter + 1}: €${cf.totalRecovery.toFixed(1)}M (${cf.stateGuaranteeRecovery > 0 ? 'SG' : ''}${cf.collateralRecovery > 0 ? 'COL' : ''})`);
        }
      });
...
const extractDefaultFlows = (product, quarters, productKey, totalAssetsResults) => {
  ...
  if (totalAssetsResults && totalAssetsResults.gbvDefaulted && totalAssetsResults.gbvDefaulted.byProduct) {
    ...
    if (productGbvData && productGbvData.quarterlyGrossNPL) {
      console.log(`✅ Found default flows for ${productKey}:`, productGbvData.quarterlyGrossNPL.slice(0, 10));
  ...
  if (product.calculatedFlows && product.calculatedFlows.defaultFlows) {
    console.log(`✅ Found calculated flows for ${productKey}:`, product.calculatedFlows.defaultFlows.slice(0, 10));
  ...
  if (product.vintages && product.vintages.length > 0) {
    ...
    if (hasDefaults) {
      console.log(`✅ Extracted from vintages for ${productKey}:`, defaultFlows.slice(0, 10));
      return defaultFlows;
    }
  }
  
  // Return empty array if no default data found
  console.log(`⚠️ No default flows found for ${productKey}`);
