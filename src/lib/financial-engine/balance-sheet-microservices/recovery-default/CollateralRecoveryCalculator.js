...
export const calculateCollateralRecovery = (defaultedAmount, product, assumptions, defaultQuarter, totalQuarters) => {
  ...
  console.log(`🔍 Product Recovery Debug:`, {
    productName: product.name,
    isUnsecured: isUnsecuredProduct(product),
    productSecured: product.secured,
    productIsUnsecured: product.isUnsecured,
    productSecurityType: product.securityType,
    productLTV: product.ltv,
    productLGD: product.lgd,
    productUnsecuredLGD: product.unsecuredLGD,
    productUnsecuredLgd: product.unsecuredLgd,
    calculatedLGD: getProductLGD(product)
  });
  
  if (isUnsecuredProduct(product)) {
    // UNSECURED: Usa LGD per calcolare recovery
    console.log(`📊 LGD Fields Check:`, {
      'product.lgd': product.lgd,
      'product.unsecuredLGD': product.unsecuredLGD,
      'product.unsecuredLgd': product.unsecuredLgd,
      'product.lgdUnsecured': product.lgdUnsecured,
      'product.LGD': product.LGD
    });
    ...
    console.log(`💳 Unsecured: €${defaultedAmount.toFixed(1)}M × (100% - ${lgd}% LGD) = €${cappedNetRecovery.toFixed(1)}M recovery`);
    ...
    console.log(`🏠 Secured: €${defaultedAmount.toFixed(1)}M ÷ ${ltv}% = €${collateralValue.toFixed(1)}M collateral`);
    console.log(`   → €${collateralValue.toFixed(1)}M × (1-${collateralHaircut}%) × (1-${recoveryCosts}%) = €${cappedNetRecovery.toFixed(1)}M`);
  }
...
const getCollateralRecoveryTime = (product) => {
  ...
  if (product.timeToRecover && product.timeToRecover > 0) {
    console.log(`📅 Using product-specific collateral recovery time: ${product.timeToRecover} quarters`);
  ...
  if (product.recoveryTime !== undefined) {
    console.log(`📅 Using product.recoveryTime: ${product.recoveryTime} years`);
    return product.recoveryTime;
  }
  
  if (product.collateralRecoveryTime !== undefined) {
    console.log(`📅 Using product.collateralRecoveryTime: ${product.collateralRecoveryTime} years`);
  ...
  console.log(`📅 Using fallback collateral recovery time: ${fallbackTime} years for type ${productType}`);
  return fallbackTime;
};
