...
const createStateGuaranteeDistribution = (stateGuaranteeRecovery, defaultQuarter, totalQuarters) => {
  ...
  console.log(`💰 State Guarantee ${guaranteeType.toUpperCase()}: ${cashflowSchedule.length} cashflows`, cashflowSchedule);
...
const createCollateralDistribution = (collateralRecovery, defaultQuarter, totalQuarters) => {
  ...
  console.log(`🏠 Collateral Recovery: ${cashflowSchedule.length} cashflows`, cashflowSchedule);
...
const getCollateralCashflowSchedule = (totalAmount, startQuarter, endQuarter) => {
  ...
  // Recovery avviene nel mezzo del periodo di liquidazione
  
  const totalPeriod = endQuarter - startQuarter + 1;
  let recoveryQuarter;
  
  if (totalPeriod <= 2) {
    // Periodo molto corto: recovery all'inizio
    recoveryQuarter = startQuarter;
  } else {
    // Periodo normale: recovery nel mezzo del periodo
    recoveryQuarter = startQuarter + Math.floor(totalPeriod / 2);
  }
