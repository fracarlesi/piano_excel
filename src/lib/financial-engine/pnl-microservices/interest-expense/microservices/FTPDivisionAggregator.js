...
export const aggregateDivisionFTP = (divisionFTPResults) => {
  console.log('📊 FTP Division Aggregator - Starting aggregation');
  
  const aggregatedResults = {};
  
  // Process each division
  Object.entries(divisionFTPResults).forEach(([divKey, divData]) => {
    console.log(`  💰 Aggregating division: ${divKey}`);
    ...
    Object.entries(divData.byProduct || {}).forEach(([prodKey, prodData]) => {
      console.log(`    📦 Adding product ${prodKey}:`, {
        ftpRate: prodData.ftpRate,
        hasQuarterlyData: !!prodData.quarterlyFTP,
        firstQuarterFTP: prodData.quarterlyFTP?.[0]?.toFixed(2)
      });
    ...
    // Log aggregation results for first year
    console.log(`  ✅ Division ${divKey} aggregation complete:`, {
      productCount: Object.keys(divData.byProduct || {}).length,
      Q1Total: quarterlyTotalNumbers[0]?.toFixed(2),
      Q2Total: quarterlyTotalNumbers[1]?.toFixed(2),
      Q3Total: quarterlyTotalNumbers[2]?.toFixed(2),
      Q4Total: quarterlyTotalNumbers[3]?.toFixed(2),
      Y1Total: (quarterlyTotalNumbers[0] + quarterlyTotalNumbers[1] + 
                quarterlyTotalNumbers[2] + quarterlyTotalNumbers[3]).toFixed(2)
    });
