...
export const orchestrateFTPCalculation = (creditResults, assumptions) => {
  console.log('🎯 FTP Orchestrator - Starting calculation');
  ...
  Object.entries(divisionMappings).forEach(([assumptionKey, dataKey]) => {
    ...
    console.log(`📍 Processing division ${dataKey}:`, {
  ...
  console.log('📊 FTP Orchestrator - Aggregating division totals from products');
  ...
  if (!validationBefore.isValid) {
    console.warn('⚠️ FTP totals discrepancy detected before aggregation:', validationBefore.discrepancies);
  }
  ...
  console.log('🎯 FTP Orchestrator - Calculation complete');
