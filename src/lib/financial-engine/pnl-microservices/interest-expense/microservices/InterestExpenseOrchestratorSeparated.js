...
export const orchestrateFTPCalculationSeparated = (creditResults, assumptions) => {
  console.log('🎯 FTP Orchestrator Separated - Starting calculation');
  console.log('📊 Credit Results structure:', {
    keys: Object.keys(creditResults || {}),
  ...
  Object.entries(divisionMappings).forEach(([assumptionKey, dataKey]) => {
    ...
    if (!divisionAssumptions || !divisionData) {
      console.log(`⚠️ Skipping division ${assumptionKey}/${dataKey} - no data`);
      return;
    }
    ...
    console.log(`📍 Processing division ${dataKey} with separated FTP:`, {
      hasData: !!divisionData,
      productCount: Object.keys(divisionProducts).length,
      productKeys: Object.keys(divisionProducts),
  ...
  console.log('🎯 FTP Orchestrator Separated - Calculation complete');
  console.log(`  Total FTP expense Q1: €${Math.abs(ftpResults.consolidated.quarterlyTotal[0]).toFixed(2)}M`);
  console.log(`    - Bonis FTP: €${Math.abs(ftpResults.consolidated.quarterlyTotalBonis[0]).toFixed(2)}M`);
  console.log(`    - NPL FTP: €${Math.abs(ftpResults.consolidated.quarterlyTotalNPL[0]).toFixed(2)}M`);
  console.log(`  Average FTP rate: ${ftpResults.consolidated.details.averageFTPRate.toFixed(2)}% (annualized)`);
  console.log(`    - Bonis: ${ftpResults.consolidated.details.averageFTPRateBonis.toFixed(2)}%`);
  console.log(`    - NPL: ${ftpResults.consolidated.details.averageFTPRateNPL.toFixed(2)}%`);
