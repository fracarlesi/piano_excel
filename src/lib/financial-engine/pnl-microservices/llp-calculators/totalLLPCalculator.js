...
export const calculateTotalLLP = (eclMovementResults, creditImpairmentResults, assumptions, quarters = 40) => {
  console.log('💰 Total LLP Calculator - Start');
  console.log('  - ECL Movement data available:', !!eclMovementResults);
  console.log('  - Credit Impairment data available:', !!creditImpairmentResults);
  ...
  for (let q = 0; q < quarters; q++) {
    ...
    if ((q < 4 || q % 4 === 0) && (eclMovement !== 0 || creditImpairment !== 0)) {
      console.log(`  Q${q}: ECL: €${eclMovement.toFixed(2)}M, Impairment: €${creditImpairment.toFixed(2)}M, Total: €${totalLLP.toFixed(2)}M`);
    }
  }
  ...
  console.log('\n💰 Total LLP Calculator - Summary');
  console.log(`  - Total LLP: €${results.metrics.totalLLP.toFixed(1)}M`);
  console.log(`  - ECL Movement: €${results.metrics.totalECLMovement.toFixed(1)}M (${results.metrics.eclPercentage.toFixed(1)}%)`);
  console.log(`  - Credit Impairment: €${results.metrics.totalCreditImpairment.toFixed(1)}M (${results.metrics.impairmentPercentage.toFixed(1)}%)`);
  console.log(`  - Peak LLP: €${Math.abs(results.metrics.peakLLP).toFixed(1)}M in Q${results.metrics.peakQuarter}`);
