...
export const calculateDivisionInterestIncomeTotals = (divisionProducts, divisionKey) => {
  console.log(`📊 Division Interest Income Totals - Processing ${divisionKey}`);
  ...
  console.log(`  📊 Division ${divisionKey} - Year 1 Summary:`);
  console.log(`     Q1 Total: €${result.total[0].toFixed(2)}M`);
  console.log(`     Q1 Performing: €${result.performingSubtotal[0].toFixed(2)}M`);
  console.log(`     Q1 NPL: €${result.nplSubtotal[0].toFixed(2)}M`);
...
export const verifyTotals = (result) => {
  ...
  if (errors.length > 0) {
    console.warn('⚠️ Interest Income Totals verification failed:', errors);
  } else {
    console.log('✅ Interest Income Totals verified successfully');
  }
