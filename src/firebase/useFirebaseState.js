    ...
    const initializeFirebase = async () => {
      try {
        ...
        const unsubscribe = onValue(assumptionsRef.current, (snapshot) => {
          ...
          if (data) {
            // Debug logs
              firebaseVersion: data.version,
              localVersion: defaultAssumptions.version,
              needsUpdate: compareVersions(defaultAssumptions.version, data.version) > 0
            });
            
            // Log Real Estate products specifically
            const reProducts = Object.entries(data.products || {})
              .filter(([key]) => key.toLowerCase().startsWith('re'))
              .map(([key, prod]) => ({ key, name: prod.name }));
                ...
                if (firebaseData.products) {
                  ...
                  Object.keys(firebaseData.products).forEach(productKey => {
                    ...
                    if (defaultProduct && typeof firebaseProduct === 'object') {
                      // For version updates, sync important fields from defaults
                      const fieldsToUpdate = ['name', 'productType', 'type'];
                      ...
                      // Update specific fields that should sync with defaults
                      fieldsToUpdate.forEach(field => {
                        if (defaultProduct[field] && defaultProduct[field] !== firebaseProduct[field]) {
                            old: firebaseProduct[field],
                            new: defaultProduct[field]
                          });
                          merged.products[productKey][field] = defaultProduct[field];
                        }
                      });
